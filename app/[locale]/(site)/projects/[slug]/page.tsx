...
async function getRelatedProjects(slug: string, locale: string) {
  return (
...
export async function generateMetadata({
  ...
  if (!project) {
    return {
      title: dict.projects.notFoundTitle,
      description: dict.projects.notFoundDescription,
      robots: {
        index: false,
        follow: false,
      },
  ...
  const description =
    project.description ||
    dict.projects.defaultProjectDescription.replace("{title}", project.title);
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${p}/projects/${project.slug}/`,
    },
    openGraph: {
      type: "article",
      title: project.title,
      description,
      url: `${p}/projects/${project.slug}/`,
      images: [
        {
          url: image,
...
export default async function ProjectDetailPage({
  ...
  const typeLabel = project.types?.length
    ...
    : "";
  const pageUrl = `${siteOrigin}${p}/projects/${project.slug}/`;
    ...
    <main className='min-h-screen px-5 pb-8 desk:pl-[260px] desk:px-10'>
      <BreadcrumbJsonLd
        ...
        items={[
          ...
          { name: dict.nav.projects, item: `${siteOrigin}${p}/projects/` },
              ...
              {relatedProjects.map((relatedProject) => {
                ...
                return (
                  <Link
                    ...
                    href={`${p}/projects/${relatedProject.slug}`}
