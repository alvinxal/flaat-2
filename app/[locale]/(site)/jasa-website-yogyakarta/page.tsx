...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
  title: "Jasa Website Yogyakarta",
  description:
    "Flaat Studio melayani jasa pembuatan website untuk bisnis di Yogyakarta, termasuk company profile, landing page, website custom, SEO basic, dan integrasi WhatsApp.",
  alternates: {
    canonical: "/jasa-website-yogyakarta/",
  },
  openGraph: {
    title: "Jasa Website Yogyakarta | Flaat Studio",
    description:
      "Bangun website profesional untuk bisnis di Yogyakarta bersama Flaat Studio.",
    url: "/jasa-website-yogyakarta/",
  },
  twitter: {
    title: "Jasa Website Yogyakarta | Flaat Studio",
    description:
...
const audiences = [
  "Startup dan bisnis kreatif",
  "Hotel, villa, dan hospitality",
  "Sekolah, kampus, dan lembaga edukasi",
  "Klinik, salon, dan jasa profesional",
  "Properti dan arsitek",
  "Kuliner, cafe, dan restoran",
];

const processSteps = [
  { step: "01", title: "Pahami", desc: "Diskusi kebutuhan bisnis dan target audiens di Yogyakarta." },
...
const benefits = [
  "Strategi digital yang disesuaikan dengan bisnis lokal Yogyakarta",
...
const jsonLd = {
  ...
  name: "Flaat Studio - Jasa Website Yogyakarta",
  url: `${siteOrigin}/jasa-website-yogyakarta/`,
  description: "Flaat Studio melayani jasa pembuatan website untuk bisnis di Yogyakarta, termasuk company profile, landing page, website custom, SEO basic, dan integrasi WhatsApp.",
...
const faqJsonLdItems = faqItems.map((i) => ({ question: i.q, answer: i.a }));
const breadcrumbItems = [
  { name: "Beranda", item: `${siteOrigin}/` },
  { name: "Jasa Website", item: `${siteOrigin}/` },
  { name: "Yogyakarta", item: `${siteOrigin}/jasa-website-yogyakarta/` },
];

export default async function JasaWebsiteYogyakartaPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main className="min-h-screen px-5 pb-8 pt-[72px] desk:pt-0 desk:pl-[260px] desk:px-10">
      ...
      <FAQJsonLd scriptId="yogyakarta-faq-jsonld" questions={faqJsonLdItems} />
      <BreadcrumbJsonLd scriptId="yogyakarta-breadcrumb-jsonld" items={breadcrumbItems} />
      <div className="relative w-full max-w-[1300px] mx-auto flex flex-col gap-[7.5rem] pt-10 px-5 tab:p-8 desk:p-8 desk:border-r desk:border-gray-200">
        <section className="relative flex flex-col gap-8 bg-[#fafafa] p-8 tab:p-12 desk:p-16">
          <p className="m-0 font-mono text-xs tracking-widest uppercase text-gray-800">JASA WEBSITE YOGYAKARTA</p>
          <div className="flex flex-col gap-10 tab:flex-row tab:items-end tab:justify-between">
            <div className="flex flex-col gap-6 max-w-[55ch]">
              <h1 className="m-0 text-3xl tab:text-4xl desk:text-5xl leading-tight tracking-tight font-medium font-sans text-accent">Jasa Website di Yogyakarta</h1>
              <p className="m-0 text-lg leading-[1.7] tracking-[-0.01em] font-body text-gray-500">Flaat Studio membantu bisnis di Yogyakarta membangun website profesional untuk meningkatkan kredibilitas dan memudahkan calon customer menemukan bisnis Anda secara online.</p>
            </div>
            <a href="https://wa.me/6285156652910?text=Halo%20Flaat%2C%20saya%20ingin%20konsultasi%20website%20untuk%20bisnis%20di%20Yogyakarta" target="_blank" rel="noreferrer" className="inline-flex items-center gap-3 w-fit pb-[0.3rem] border-b border-accent text-accent no-underline text-lg leading-[1.3] tracking-[-0.02em] font-sans transition-opacity duration-250 hover:opacity-60 shrink-0"><span>Konsultasi Gratis</span><span aria-hidden="true">→</span></a>
          </div>
        </section>
        <section className="flex flex-col gap-6">
          <p className="m-0 font-mono text-xs tracking-widest uppercase text-gray-800">KEBUTUHAN</p>
          <h2 className="m-0 text-2xl tab:text-3xl leading-tight font-semibold font-sans text-accent max-w-[28ch]">Website yang membantu bisnis Jogja terlihat lebih profesional</h2>
          <p className="m-0 text-lg leading-[1.7] tracking-[-0.01em] font-body text-gray-500 max-w-[70ch]">Banyak bisnis di Yogyakarta sudah aktif di media sosial, tapi belum punya website sebagai pusat informasi resmi. Website profesional membantu bisnis Anda lebih kredibel, mudah ditemukan di Google, dan punya satu tempat untuk semua informasi penting.</p>
        </section>
        <section className="flex flex-col gap-6">
        ...
        <section className="flex flex-col gap-6">
          ...
          <h2 className="m-0 text-2xl tab:text-3xl leading-tight font-semibold font-sans text-accent">Bisnis di Yogyakarta yang cocok</h2>
        ...
        <section className="flex flex-col gap-8 bg-[#2f4157] p-8 tab:p-12 desk:p-16">
          <h2 className="m-0 text-2xl tab:text-3xl leading-tight font-semibold font-sans text-white max-w-[24ch]">Siap membuat website untuk bisnis di Yogyakarta?</h2>
