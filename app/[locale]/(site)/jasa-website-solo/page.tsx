...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
  title: "Jasa Website Solo",
  description:
    "Flaat Studio melayani jasa pembuatan website untuk bisnis di Solo, termasuk company profile, landing page, website custom, SEO basic, dan integrasi WhatsApp.",
  alternates: {
    canonical: "/jasa-website-solo/",
  },
  openGraph: {
    title: "Jasa Website Solo | Flaat Studio",
    description:
      "Bangun website profesional untuk bisnis di Solo bersama Flaat Studio.",
    url: "/jasa-website-solo/",
  },
  twitter: {
    title: "Jasa Website Solo | Flaat Studio",
    description:
...
const audiences = [
  ...
  "Batik dan fashion lokal",
  "Jasa profesional dan konsultan",
  "Properti dan properti",
];

const processSteps = [
  { step: "01", title: "Pahami", desc: "Diskusi kebutuhan bisnis dan target audiens di Solo." },
...
const benefits = [
  "Strategi digital yang disesuaikan dengan bisnis lokal Solo",
...
const faqItems = [
  { q: "Apakah Flaat Studio melayani jasa website di Solo?", a: "Ya. Flaat Studio melayani pembuatan website untuk bisnis di Solo (Surakarta) dan berbagai kota lain seperti Yogyakarta, Semarang, Temanggung, Purwokerto, Magelang, dan Salatiga." },
...
const jsonLd = {
  ...
  name: "Flaat Studio - Jasa Website Solo",
  url: `${siteOrigin}/jasa-website-solo/`,
  description: "Flaat Studio melayani jasa pembuatan website untuk bisnis di Solo (Surakarta), termasuk company profile, landing page, website custom, SEO basic, dan integrasi WhatsApp.",
  areaServed: { "@type": "City", name: "Solo" },
...
export default async function JasaWebsiteSoloPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main className="min-h-screen px-5 pb-8 pt-[72px] desk:pt-0 desk:pl-[260px] desk:px-10">
      ...
      <FAQJsonLd scriptId="solo-faq-jsonld" questions={faqItems.map((i) => ({ question: i.q, answer: i.a }))} />
      <BreadcrumbJsonLd
        scriptId="solo-breadcrumb-jsonld"
        items={[
          { name: "Beranda", item: `${siteOrigin}/` },
          { name: "Jasa Website", item: `${siteOrigin}/` },
          { name: "Solo", item: `${siteOrigin}/jasa-website-solo/` },
        ]}
      />
      <div className="relative w-full max-w-[1300px] mx-auto flex flex-col gap-[7.5rem] pt-10 px-5 tab:p-8 desk:p-8 desk:border-r desk:border-gray-200">
        <section className="relative flex flex-col gap-8 bg-[#fafafa] p-8 tab:p-12 desk:p-16">
          <p className="m-0 font-mono text-xs tracking-widest uppercase text-gray-800">JASA WEBSITE SOLO</p>
          <div className="flex flex-col gap-10 tab:flex-row tab:items-end tab:justify-between">
            <div className="flex flex-col gap-6 max-w-[55ch]">
              <h1 className="m-0 text-3xl tab:text-4xl desk:text-5xl leading-tight tracking-tight font-medium font-sans text-accent">Jasa Website di Solo</h1>
              <p className="m-0 text-lg leading-[1.7] tracking-[-0.01em] font-body text-gray-500">Flaat Studio membantu bisnis di Solo (Surakarta) membangun website profesional untuk meningkatkan kredibilitas dan memudahkan calon customer menemukan bisnis Anda secara online.</p>
            </div>
            <a href="https://wa.me/6285156652910?text=Halo%20Flaat%2C%20saya%20ingin%20konsultasi%20website%20untuk%20bisnis%20di%20Solo" target="_blank" rel="noreferrer" className="inline-flex items-center gap-3 w-fit pb-[0.3rem] border-b border-accent text-accent no-underline text-lg leading-[1.3] tracking-[-0.02em] font-sans transition-opacity duration-250 hover:opacity-60 shrink-0"><span>Konsultasi Gratis</span><span aria-hidden="true">→</span></a>
          </div>
        </section>
        <section className="flex flex-col gap-6">
          <p className="m-0 font-mono text-xs tracking-widest uppercase text-gray-800">KEBUTUHAN</p>
          <h2 className="m-0 text-2xl tab:text-3xl leading-tight font-semibold font-sans text-accent max-w-[28ch]">Website yang membantu bisnis Solo terlihat lebih profesional</h2>
          <p className="m-0 text-lg leading-[1.7] tracking-[-0.01em] font-body text-gray-500 max-w-[70ch]">Banyak bisnis di Solo sudah aktif di media sosial, tapi belum punya website sebagai pusat informasi resmi. Website profesional membantu bisnis Anda lebih kredibel, mudah ditemukan di Google, dan punya satu tempat untuk semua informasi penting.</p>
        </section>
        <section className="flex flex-col gap-6">
        ...
        <section className="flex flex-col gap-6">
          ...
          <h2 className="m-0 text-2xl tab:text-3xl leading-tight font-semibold font-sans text-accent">Bisnis di Solo yang cocok</h2>
        ...
        <section className="flex flex-col gap-8 bg-[#2f4157] p-8 tab:p-12 desk:p-16">
          <h2 className="m-0 text-2xl tab:text-3xl leading-tight font-semibold font-sans text-white max-w-[24ch]">Siap membuat website untuk bisnis di Solo?</h2>
          ...
          <a href="https://wa.me/6285156652910?text=Halo%20Flaat%2C%20saya%20ingin%20konsultasi%20website%20untuk%20bisnis%20di%20Solo" target="_blank" rel="noreferrer" className="inline-flex items-center gap-3 w-fit pb-[0.3rem] border-b border-white/50 text-white no-underline text-lg leading-[1.3] tracking-[-0.02em] font-sans transition-opacity duration-250 hover:opacity-70"><span>Konsultasi Gratis via WhatsApp</span><span aria-hidden="true">→</span></a>
        </section>
        <section className="flex flex-col gap-6">
